...
async def bom_root(product_unit, request: Request):
    ...
    response = postgresql.session.query(BOM).filter(BOM.product_unit==product_unit).order_by(asc(BOM.process_order)).all()
...
@router.delete("/bom/{product_unit}/{process_order}", status_code=200)
async def bom_root(product_unit, process_order, request: Request):
    # 1. Check Request
    if product_unit and process_order is None:
        raise HTTPException(status_code=400, detail="Bad Request(uri missing)")
    
    # 2. Execute Business Logic
    response = postgresql.session.query(BOM).filter(and_(BOM.product_unit==product_unit, BOM.process_order==process_order)).first()
    postgresql.session.delete(response)
    postgresql.session.commit()

    response = postgresql.session.query(BOM).filter(and_(BOM.product_unit == product_unit, BOM.process_order > process_order)).order_by(asc(BOM.process_order)).all()
