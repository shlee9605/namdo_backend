...
async def input(params):
    ...
    if result is not None:
        raise HTTPException(status_code=404, detail="Existing Process Name")
...
async def edit(params):
    ...
    if result is not None:
        raise HTTPException(status_code=404, detail="Already Existing New Process Data")

    # 2. find data
    result = await process_dao.read(params['old_process_name'])
    if result is None:
        raise HTTPException(status_code=404, detail="No Existing Old Process Data")
